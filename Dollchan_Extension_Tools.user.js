...
(function deMainFuncInner(scriptStorage, FormData, scrollTo, localData) {
	...
	var runMain = function () {
		var _ref78 = _asyncToGenerator( regeneratorRuntime.mark(function _callee22(checkDomains, dataPromise) {
			var formEl, favObj, oldMain, _ref79, _ref80, storageName, firstThr;
			...
			return regeneratorRuntime.wrap(function _callee22$(_context27) {
				while (1) {
					switch (_context27.prev = _context27.next) {
						...
						case 15:
							_ref79 = _context27.sent;
							_ref80 = _slicedToArray(_ref79, 2);
							excludeList = _ref80[0];
							favObj = _ref80[1];
		...
		return function runMain(_x82, _x83) {
	...
	function showFavoritesWindow(body, favObj) {
		...
		btns.appendChild($btn(Lng.page[lang], Lng.infoPage[lang], _asyncToGenerator( regeneratorRuntime.mark(function _callee6() {
			var els, len, thrInfo, i, el, iconEl, titleEl, endPage, infoLoaded, page, tNums, form, _i, pInfo, _i2, _thrInfo$_i, found, pageEl, iconClass, _iconEl, iconTitle, _titleEl;
					...
					switch (_context7.prev = _context7.next) {
						...
						case 28:
							for (_i2 = 0; _i2 < len; ++_i2) {
								_thrInfo$_i = thrInfo[_i2], found = _thrInfo$_i.found, pageEl = _thrInfo$_i.pageEl, iconClass = _thrInfo$_i.iconClass, _iconEl = _thrInfo$_i.iconEl, iconTitle = _thrInfo$_i.iconTitle, _titleEl = _thrInfo$_i.titleEl;
...
'-',,,,,,,,,,,,, ';', '=', ',', '-', '.', '/', '`',,,,,,,,,,,,,,,,,,,,,,,,,,, '[', '\\', ']', "'"];
		...
		_createClass(VideosParser, [{
			...
			value: function parse(data) {
				...
				if (Cfg.addVimeo) {
					...
					for (var _i3 = 0, _len4 = links.length; _i3 < _len4; ++_i3) {
						var _link = links[_i3];
				...
				for (var _i4 = 0, _len5 = vids.length; _i4 < _len5; ++_i4) {
					var _vids$_i = _slicedToArray(vids[_i4], 3),
					    post = _vids$_i[0],
					    _m2 = _vids$_i[1],
					    isYtube = _vids$_i[2];
	...
	function embedAudioLinks(data) {
		...
		if (Cfg.addVocaroo) {
			...
			for (var _i5 = 0, _len6 = _els.length; _i5 < _len6; ++_i5) {
				var _link2 = _els[_i5];
...
true, true];
		...
		_sort: function _sort(sp) {
			...
			sp = sp.sort();
			for (var _i6 = 0, _len7 = sp.length - 1; _i6 < _len7; ++_i6) {
				var j = _i6 + 1;
				if (sp[_i6][0] === sp[j][0] && sp[_i6][1] <= sp[j][1] && sp[_i6][1] >= sp[j][1] && (sp[_i6][2] === null || 
				sp[_i6][2] === undefined || 
				sp[_i6][2] <= sp[j][2] && sp[_i6][2] >= sp[j][2])) {
					sp.splice(j, 1);
					_i6--;
					_len7--;
				} else if (sp[_i6][0] === 0xFF) {
					sp.push(sp.splice(_i6, 1)[0]);
					_i6--;
					_len7--;
				}
			}
			...
			value: function setUserPassw() {
				...
				try {
					for (var _iterator22 = DelForm[Symbol.iterator](), _step22; !(_iteratorNormalCompletion22 = (_step22 = _iterator22.next()).done); _iteratorNormalCompletion22 = true) {
						...
						var _ref38$passEl = _ref38.passEl;
						var passEl = _ref38$passEl === undefined ? {} : _ref38$passEl;
	...
	var AbstractPost = function () {
		...
		_createClass(AbstractPost, [{
			...
			value: function handleEvent(e) {
				...
				if (type === 'click') {
					...
					switch (el.classList[0]) {
						...
						case 'de-btn-fav':
							this.thr.setFavorState(true, isPview ? this : null);return;
						case 'de-btn-fav-sel':
							this.thr.setFavorState(false, isPview ? this : null);return;
						case 'de-btn-hide':
			...
			key: 'setFavBtn',
			value: function setFavBtn(isEnable) {
				var el = $q(isEnable ? '.de-btn-fav' : '.de-btn-fav-sel', this.btns);
				if (el) {
			...
			value: function _getFullMsg(el, isInit) {
				...
				ajaxLoad(aib.getThrUrl(aib.b, this.tNum)).then(function (form) {
					var maybeSpells = new Maybe(SpellsRunner);
					if (_this44.isOp) {
						_this44.updateMsg(aib.fixHTML(doc.adoptNode($q(aib.qPostMsg, form))), maybeSpells.value);
						$del(el);
					} else {
						var _els3 = $Q(aib.qRPost, form);
						for (var i = 0, len = _els3.length; i < len; ++i) {
							if (_this44.num === aib.getPNum(_els3[i])) {
								_this44.updateMsg(aib.fixHTML(doc.adoptNode($q(aib.qPostMsg, _els3[i]))), maybeSpells.value);
								$del(el);
								break;
							}
						}
	...
	var Post = function (_AbstractPost) {
		...
		function Post(el, thr, num, count, isOp, prev) {
			...
			_this46.btns = $aEnd(_this46._pref = $q(aib.qPostRef, el), '<span class="de-post-btns' + (isOp ? '' : ' de-post-counter') + '">' + '<svg class="de-btn-hide"><use class="de-btn-hide-use" xlink:href="#de-symbol-post-hide"/>' + '<use class="de-btn-unhide-use" xlink:href="#de-symbol-post-unhide"/></svg>' + '<svg class="de-btn-rep"><use xlink:href="#de-symbol-post-rep"/></svg>' + (isOp ? (aib.t ? '' : '<svg class="de-btn-expthr"><use xlink:href="#de-symbol-post-expthr"/></svg>') + '<svg class="de-btn-fav"><use xlink:href="#de-symbol-post-fav"/></svg>' : '') + (_this46.sage ? '<svg class="de-btn-sage"><use xlink:href="#de-symbol-post-sage"/></svg>' : '') + '</span>');
	...
	var Pview = function (_AbstractPost2) {
		...
		_createClass(Pview, [{
			...
			key: '_buildPview',
			value: function _buildPview(post) {
				var _this52 = this;

				$del(this.el);
				var num = this.num;

				var isMyPost = Cfg.markMyPosts && MyPosts.has(num);
				var pv = this.el = post.el.cloneNode(true);
				pByEl.set(pv, this);
				pv.className = aib.cReply + ' de-pview' + (post.isViewed ? ' de-viewed' : '') + (isMyPost ? ' de-mypost' : '');
				$show(pv);
				...
				var pText = '<svg class="de-btn-rep"><use xlink:href="#de-symbol-post-rep"/></svg>' + (isOp ? '<svg class="' + (post.thr.isFav || function () {
					var _ref50 = _asyncToGenerator( regeneratorRuntime.mark(function _callee16(f) {
						return regeneratorRuntime.wrap(function _callee16$(_context17) {
							while (1) {
								switch (_context17.prev = _context17.next) {
									...
									case 2:
										_context17.t2 = aib.host;
										_context17.t1 = f = _context17.sent[_context17.t2];
				...
				}()() ? 'de-btn-fav-sel' : 'de-btn-fav') + '"><use xlink:href="#de-symbol-post-fav"></use></svg>' : '') + (post.sage ? '<svg class="de-btn-sage"><use xlink:href="#de-symbol-post-sage"/></svg>' : '') + '<svg class="de-btn-stick"><use xlink:href="#de-symbol-post-stick"/></svg>' + (post.isDeleted ? '' : '<span class="de-post-counter-pview">' + (isOp ? 'OP' : post.count + +!aib.JsonBuilder) + (isMyPost ? ' (You)' : '') + '</span>');
				if (post instanceof CacheItem) {
					...
					var btnsEl = this.btns = this._pref.nextSibling;
	...
	var CacheItem = function () {
		...
		_createClass(CacheItem, [{
			...
			key: 'title',
			get: function get() {
				var value = new Post.Сontent(this).title;
				Object.defineProperty(this, 'title', { value: value });
				return value;
			}
		}]);
	...
	var ImagesViewer = function () {
		...
		_createClass(ImagesViewer, [{
			...
			value: function _showFullImg(data) {
				...
				var el = $add('<div class="de-fullimg-center" style="top:' + (this._oldT - (Cfg.imgInfoLink ? 11 : 0)) + 'px; left:' + this._oldL + 'px; width:' + width + 'px; height:' + height + 'px; display: block"></div>');
			...
			value: function getFullImg(inPost, onsizechange, onrotate) {
				...
				if (!this.isVideo) {
					...
					img.onload = img.onerror = function (_ref52) {
						...
						if (!_this58._size || isExifRotated) {
							_this58._size = isExifRotated ? [newH, newW] : [newW, newH];
						}
	...
	var PostImages = function () {
		function PostImages(post) {
			...
			if (Cfg.addImgs || localData) {
				...
				for (var _i10 = 0, _len8 = els.length; _i10 < _len8; ++_i10) {
					var _el8 = els[_i10];
	...
	var Thread = function () {
		function Thread(el, num, prev, form) {
			...
			updBtn.onclick = function (e) {
				$pd(e);
				_this62.loadPosts('new');
			};
			if (Cfg.hideReplies) {
				var repBtn = $bEnd(this.btns, ' <span class="de-replies-btn">[<a class="de-abtn" href="#"></a>]</span>');
				repBtn.onclick = function (e) {
					$pd(e);
					var nextCoord = !_this62.next || _this62.last.isOmitted ? null : _this62.next.top;
					_this62._toggleReplies(repBtn, updBtn);
					if (nextCoord) {
						scrollTo(window.pageXOffset, window.pageYOffset + _this62.next.top - nextCoord);
					}
				};
		...
		_createClass(Thread, [{
			...
			key: 'setFavorState',
			value: function setFavorState(isEnable, preview) {
				var h = void 0,
				    b = void 0,
				    num = void 0,
				    cnt = void 0,
			...
			value: function _importPosts(last, pBuilder, begin, end, maybeVParser, maybeSpells) {
				...
				if (aib.JsonBuilder && nav.hasTemplate) {
					...
					for (var _i12 = 0, _len9 = _posts.length; _i12 < _len9; ++_i12) {
				...
				while (true) {
					switch (this._state) {
						...
						case 1:
							counter.setWait();
							this._state = 2;
							this._loadPromise = Thread.first.loadNewPosts().then(function (_ref66) {
								var newCount = _ref66.newCount,
								    locked = _ref66.locked;
	...
	function getImageBoard(checkDomains, checkEngines) {
			...
			_createClass(Makaba, [{
				...
				value: function initCaptcha(cap) {
					...
					if (Cfg.altCaptcha && box.firstChild.tagName !== 'IMG') {
						...
						var _ref67 = [].concat(_toConsumableArray(box.children)),
						    img = _ref67[0],
						    inp = _ref67[1];
		...
		var Tinyboard = function (_BaseBoard2) {
			...
			_createClass(Tinyboard, [{
				key: 'changeReplyMode',
				value: function () {
					var _ref68 = _asyncToGenerator( regeneratorRuntime.mark(function _callee18(form, tNum) {
						var _this80 = this;
					...
					function changeReplyMode(_x73, _x74) {
				...
				key: 'getSubmitData',
				value: function getSubmitData(_ref69) {
		...
		var LynxChan = function (_BaseBoard5) {
			...
			_createClass(LynxChan, [{
				...
				key: 'sendHTML5Post',
				value: function () {
					var _ref71 = _asyncToGenerator( regeneratorRuntime.mark(function _callee21(form, data, needProgress, hasFiles) {
						var _this85 = this;
						...
						return regeneratorRuntime.wrap(function _callee21$(_context26) {
							while (1) {
								switch (_context26.prev = _context26.next) {
									case 0:
										getBase64 = function () {
											var _ref72 = _asyncToGenerator( regeneratorRuntime.mark(function _callee19(file) {
											...
											return function getBase64(_x79) {
										...
										data.forEach(function () {
											var _ref73 = _asyncToGenerator( regeneratorRuntime.mark(function _callee20(value, key) {
					...
					function sendHTML5Post(_x75, _x76, _x77, _x78) {
		...
		var _2chRip = function (_BaseBoard8) {
			...
			_createClass(_2chRip, [{
				...
				key: 'updateCaptcha',
				value: function updateCaptcha(cap) {
					return cap.updateHelper('/cgi/captcha?task=get_id', function (_ref74) {
						var id = _ref74.responseText;
		...
		var Krautchan = function (_BaseBoard16) {
			...
			function Krautchan(prot, dm) {
				...
				var _this105 = _possibleConstructorReturn(this, (Krautchan.__proto__ || Object.getPrototypeOf(Krautchan)).call(this, prot, dm));

				_this105.cReply = 'postreply';
				_this105.qBan = '.ban_mark';
				_this105.qClosed = 'img[src="/images/locked.gif"]';
				_this105.qDForm = 'form[action*="delete"]';
				_this105.qError = '.message_text';
				_this105.qFormRedir = 'input#forward_thread';
				_this105.qFormRules = '#rules_row';
				_this105.qImgInfo = '.fileinfo';
				_this105.qOmitted = '.omittedinfo';
				_this105.qPages = 'table[border="1"] > tbody > tr > td > a:nth-last-child(2) + a';
				_this105.qPostHeader = '.postheader';
				_this105.qPostImg = 'img[id^="thumbnail_"]';
				_this105.qPostRef = '.postnumber';
				_this105.qPostSubj = '.postsubject';
				_this105.qRPost = '.postreply';
				_this105.qTrunc = 'p[id^="post_truncated"]';
				...
				_this105.res = 'thread-';
				_this105.timePattern = 'yyyy+nn+dd+hh+ii+ss+--?-?-?-?-?';
				return _this105;
			}
	...
	var DollchanAPI = {
		...
		_handleMessage: function _handleMessage(_ref76) {
			var arg = _ref76.data;
	...
	function checkForUpdates(isManual, lastUpdateTime) {
		...
		return $ajax(gitRaw + 'src/modules/Wrap.js', { 'Content-Type': 'text/plain' }, false).then(function (_ref77) {
			var responseText = _ref77.responseText;

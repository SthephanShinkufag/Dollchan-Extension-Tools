...
(function de_main_func_inner(scriptStorage, FormData) {
					...
					switch (el.id) {
						...
						case 'de-panel-audio-off':
							...
							this._menuTO = setTimeout(function () {
								_this3._menu = addMenu(el);
								_this3._menu.onover = function () {
									return clearTimeout(_this3._hideTO);
								};
								_this3._menu.onout = function () {
									return _this3._prepareToHide(null);
								};
								_this3._menu.onremove = function () {
									return _this3._menu = null;
								};
							}, Cfg.linksOver);
					}
					return;
		...
		return parent.appendChild($New('div', { 'class': 'de-content-block' }, [$new('input', { 'type': 'checkbox' }, {
			'click': function click() {
				...
				$each($Q('.de-entry > input', this.parentNode), function (el) {
					return el.checked = _this4.checked;
				});
			}
		}), title]));
	}
		...
		enable: function enable() {
			...
			if (!this.enabled) {
				...
				Promise.resolve(this.readKeys()).then(function (keys) {
					if (_this6.enabled) {
						_this6.gKeys = keys[2];
						_this6.ntKeys = keys[3];
						_this6.tKeys = keys[4];
						doc.addEventListener('keydown', _this6, true);
					}
				});
			}
		},
	...
	DateTime.prototype = {
		...
		fix: function fix(txt) {
			...
			return txt.replace(new RegExp(this.regex, 'g'), function (str) {
				...
				dtime.setHours(dtime.getHours() + _this8.diff);
				return _this8.genDateTime(dtime);
			});
		}
	};
	...
	var Pages = {
		add: function add() {
			...
			this._addPromise = ajaxLoad(aib.getPageUrl(aib.b, pageNum)).then(function (formEl) {
				_this9._addForm(formEl, pageNum);
				return spawn(_this9._updateForms, DelForm.last);
			}).then(function () {
				return _this9._endAdding();
			})['catch'](function (e) {
		...
		hasNumSpell: false,
			...
			if (this._endPromise) {
				this._endPromise.then(function () {
					return _this10._savePostsHelper();
				});
			} else {
		...
		run: function run(post) {
			...
			if (res instanceof Promise) {
				res = res.then(function (val) {
					return _this11._checkRes(post, val);
				});
	...
	function PostForm(form, ignoreForm, dc) {
		...
		el.firstChild.onclick = function () {
			...
			if (Cfg.replyWinDrag) {
				_this12.qArea.className = aib.cReply + ' de-win';
				updateWinZ(_this12.qArea.style);
			} else {
				_this12.qArea.className = aib.cReply + ' de-win-inpost';
				_this12.txta.focus();
			}
		};
		...
		this.subm.addEventListener('click', function (e) {
			if (Cfg.warnSubjTrip && _this12.subj && /#.|##./.test(_this12.subj.value)) {
				$pd(e);
				$popup(Lng.subjHasTrip[lang], 'upload', false);
				return;
			}
			var val = _this12.txta.value;
			if (Spells.outreps) {
				val = Spells.outReplace(val);
			}
			if (_this12.tNum && pByNum.get(_this12.tNum).subj === 'Dollchan Extension Tools') {
				var temp = '\n\n' + _this12._wrapText(aib.markupBB, aib.markupTags[5], '-'.repeat(50) + '\n' + nav.ua + '\nv' + version + '.' + commit + ' [' + nav.scriptInstall + ']')[1];
				if (!val.includes(temp)) {
					val += temp;
				}
			}
			_this12.txta.value = val;
			if (Cfg.ajaxReply) {
				$popup(Lng.checking[lang], 'upload', true);
			}
			if (_this12.video && (val = _this12.video.value) && (val = val.match(Videos.ytReg))) {
				_this12.video.value = 'http://www.youtube.com/watch?v=' + val[1];
			}
			if (_this12.isQuick) {
				$hide(_this12.pForm);
				$hide(_this12.qArea);
				$after(_this12._pBtn[+_this12.isBottom], _this12.pForm);
			}
			updater.pause();
		});
			...
			this.form.onsubmit = function (e) {
				...
				if (aib._2chruNet) {
					...
					doc.body.lastChild.onload = function (e) {
						$del(e.target);
						spawn(html5Submit, _this12.form, _this12.subm, true).then(doUploading);
					};
					return;
				}
				spawn(html5Submit, _this12.form, _this12.subm, true).then(doUploading);
	...
	FileInput.prototype = {
		...
		handleEvent: function handleEvent(e) {
			...
			switch (e.type) {
				...
				case 'drop':
					setTimeout(function () {
						_this13.thumb.classList.remove('de-file-drag');
						var el = _this13.place.firstChild;
						if (el) {
							$before(el, _this13.el);
						} else {
							_this13.place.appendChild(_this13.el);
						}
					}, 10);
					return;
				case 'mouseover':
		...
		init: function init(update) {
			...
			if (Cfg.fileThumb) {
				setTimeout(function () {
					$hide(_this14.form.fileTd.parentNode);
				}, 0);
		...
		_addRarJpeg: function _addRarJpeg() {
			...
			el.onchange = function (e) {
				$del(_this15._rjUtil);
				_this15._buttonsPlace.insertAdjacentHTML('afterend', '<span><svg class="de-wait"><use xlink:href="#de-symbol-wait"/></svg>' + Lng.wait[lang] + '</span>');
				var myRjUtil = _this15._rjUtil = _this15._buttonsPlace.nextSibling,
				    file = e.target.files[0];
				readFile(file, false).then(function (val) {
					if (_this15._rjUtil === myRjUtil) {
						myRjUtil.className = 'de-file-rarmsg de-file-utils';
						myRjUtil.title = _this15.el.files[0].name + ' + ' + file.name;
						myRjUtil.textContent = _this15.el.files[0].name.replace(/^.+\./, '') + ' + ' + file.name.replace(/^.+\./, '');
						_this15.imgFile = val;
					}
				});
			};
		...
		_showPviewImage: function _showPviewImage() {
			...
			readFile(files[0], false).then(function (val) {
				_this16.form.eventFiles(false);
				if (_this16.empty) {
					return;
				}
				var file = _this16.el.files[0],
				    thumb = _this16.thumb;
				...
				_this16._mediaEl = thumb = thumb.firstChild;
		...
		_createClass(Captcha, [{
			...
			value: function add() {
				...
				if (initPromise) {
					initPromise.then(function () {
						return _this17.initCaptcha(focus, false);
					}, function (e) {
						if (e instanceof AjaxError) {
							_this17._setUpdateError(e);
						} else {
							_this17.hasCaptcha = false;
						}
					});
			...
			value: function initImage(img) {
				...
				img.onclick = function () {
					return _this18.update(true, false);
				};
			}
		}, {
			key: 'initTextEl',
				...
				if (aib.updateCaptcha) {
					...
					if (updatePromise) {
						updatePromise.then(function () {
							return _this19._updateTextEl(focus);
						}, function (e) {
							return _this19._setUpdateError(e);
						});
						return;
					}
				} else {
		...
		_createClass(AbstractPost, [{
			...
			value: function _getFull(node, isInit) {
				...
				ajaxLoad(aib.getThrdUrl(aib.b, this.tNum)).then(function (form) {
					...
					if (_this27.isOp) {
						_this27.updateMsg(replacePost($q(aib.qPostMsg, form)), maybeSpells.value);
						$del(node);
					} else {
						var els = $Q(aib.qRPost, form);
						for (var i = 0, len = els.length; i < len; i++) {
							if (_this27.num === aib.getPNum(els[i])) {
								_this27.updateMsg(replacePost($q(aib.qPostMsg, els[i])), maybeSpells.value);
								$del(node);
								break;
							}
						}
	...
	function getImageBoard(checkDomains, checkEngines) {
		...
		var Makaba = (function (_BaseBoard) {
			...
			function Makaba(prot, dm) {
				...
				_this45.cPostSubj = 'post-title';
				_this45.cReply = 'post reply';
				_this45.qBan = '.pomyanem';
				_this45.qClosed = '.sticky-img[src$="locked.png"]';
				_this45.qDForm = '#posts-form';
				_this45.qFormRedir = null;
				_this45.qFormRules = '.rules-area';
				_this45.qOmitted = '.mess-post';
				_this45.qPostImg = '.preview';
				_this45.qPostMsg = '.post-message';
				_this45.qPostName = '.ananimas, .post-email';
				_this45.qRPost = 'div.reply';
				_this45.qTrunc = null;

				_this45.hasCatalog = true;
		...
		var Futaba = (function (_BaseBoard2) {
			...
			function Futaba(prot, dm) {
				...
				_this47.qForm = 'form:nth-of-type(1)';
				_this47.qFormRedir = null;
				_this47.qFormRules = '.chui';
				_this47.qOmitted = 'font[color="#707070"]';
				_this47.qPostImg = 'a[href$=".jpg"] > img, a[href$=".png"] > img, a[href$=".gif"] > img';
				_this47.qPostRef = '.del';
		...
		var Tinyboard = (function (_BaseBoard3) {
			...
			function Tinyboard(prot, dm) {
				...
				_this48.cPostSubj = 'subject';
				_this48.cPostTrip = 'trip';
				_this48.cReply = 'post reply';
				_this48.qClosed = '.fa-lock';
				_this48.qDForm = 'form[name*="postcontrols"]';
				_this48.qForm = 'form[name="post"]';
				_this48.qFormPassw = 'input[name="password"]';
				_this48.qFormRedir = null;
				_this48.qOmitted = '.omitted';
				_this48.qPages = '.pages > a:nth-last-of-type(2)';
				_this48.qPostMsg = '.body';
				_this48.qPostName = '.name';
				_this48.qPostRef = '.post_no + a';
				_this48.qTrunc = '.toolong';

				_this48.firstPage = 1;
				_this48.hasCatalog = true;
		...
		var Vichan = (function (_Tinyboard) {
			...
			function Vichan(prot, dm) {
				...
				var _this49 = _possibleConstructorReturn(this, Object.getPrototypeOf(Vichan).call(this, prot, dm));

				_this49.qDelPassw = '#password';

				_this49.multiFile = true;
				return _this49;
			}
		...
		var Kusaba = (function (_BaseBoard4) {
			...
			function Kusaba(prot, dm) {
				...
				var _this50 = _possibleConstructorReturn(this, Object.getPrototypeOf(Kusaba).call(this, prot, dm));

				_this50.kus = true;

				_this50.qFormRedir = null;
				_this50.qError = 'h1, h2, div[style*="1.25em"]';
		...
		var _0chan = (function (_Kusaba) {
			...
			function _0chan(prot, dm) {
				...
				var _this51 = _possibleConstructorReturn(this, Object.getPrototypeOf(_0chan).call(this, prot, dm));

				_this51.cOPost = 'postnode';
				_this51.qFormRedir = '#gotothread';

				_this51.hasCatalog = true;
				_this51.ru = true;
				return _this51;
			}
		...
		var Phutaba = (function (_BaseBoard5) {
			...
			function Phutaba(prot, dm) {
				...
				_this52.cPostSubj = 'subject';
				_this52.cPostTrip = 'tripcode';
				_this52.cReply = 'post';
				_this52.qError = '.error';
				_this52.qFormRedir = 'input[name="gb2"][value="thread"]';
				_this52.qPages = '.pagelist > li:nth-last-child(2)';
				_this52.qPostMsg = '.text';
		...
		var _02chSu = (function (_Kusaba2) {
			...
			function _02chSu(prot, dm) {
				...
				var _this55 = _possibleConstructorReturn(this, Object.getPrototypeOf(_02chSu).call(this, prot, dm));
		...
		var _2chruNet = (function (_BaseBoard7) {
			...
			function _2chruNet(prot, dm) {
				...
				var _this57 = _possibleConstructorReturn(this, Object.getPrototypeOf(_2chruNet).call(this, prot, dm));

				_this57._2chruNet = true;

				_this57.qFormRedir = 'input[name="noko"]';
				_this57.qPages = '#pager > li:nth-last-child(2)';
			...
			_createClass(_2chruNet, [{
				...
				value: function updateCaptcha(cap) {
					...
					return this._capUpdPromise = $ajax('/' + this.b + '/api/requires-captcha').then(function (xhr) {
						...
						$id('captchaimage').src = '/' + _this58.b + '/captcha?' + Math.random();
						if (!$id('de-_2chruNet-capchecker')) {
							...
							cap.textEl.nextSibling.onclick = function (_ref45) {
								...
								$ajax('/' + _this58.b + '/api/validate-captcha', { method: 'POST' }).then(function (xhr) {
		...
		var _2chRu = (function (_BaseBoard8) {
			...
			function _2chRu(prot, dm) {
				...
				var _this59 = _possibleConstructorReturn(this, Object.getPrototypeOf(_2chRu).call(this, prot, dm));

				_this59._2chRu = true;

				_this59.qPages = 'table[border="1"] td > a:last-of-type';

				_this59.docExt = '.html';
				_this59.hasPicWrap = true;
				_this59.markupBB = true;
				_this59.multiFile = true;
				_this59.ru = true;

				_this59._qTable = 'table:not(.postfiles)';
				return _this59;
			}
		...
		var _410chanOrg = (function (_Kusaba3) {
			...
			function _410chanOrg(prot, dm) {
				...
				var _this60 = _possibleConstructorReturn(this, Object.getPrototypeOf(_410chanOrg).call(this, prot, dm));

				_this60.qFormRedir = 'input#noko';
				_this60.qPages = '.pgstbl > table > tbody > tr > td:nth-child(2)';

				_this60.markupBB = false;
				_this60.timePattern = 'dd+nn+yyyy++w++hh+ii+ss';
				return _this60;
			}
		...
		var _420chanOrg = (function (_BaseBoard9) {
			...
			function _420chanOrg(prot, dm) {
				...
				var _this61 = _possibleConstructorReturn(this, Object.getPrototypeOf(_420chanOrg).call(this, prot, dm));

				_this61._420 = true;

				_this61.cPostHeader = 'replyheader';
				_this61.qBan = '.ban';
				_this61.qError = 'pre';
				_this61.qFormRedir = null;
				_this61.qPages = '.pagelist > a:last-child';

	...
	function initThreadUpdater(title, enableUpdate) {
		...
		var favicon = {
			...
			initIcons: function initIcons() {
				...
				var getImg = function getImg(src, onload) {
					var img = new Image();
					img.onload = onload;
					img.src = src;
				};
				var svgToUrl = function svgToUrl(data) {
					return window.URL.createObjectURL(new Blob(['<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16">' + data + '</svg>'], { type: 'image/svg+xml;charset=utf-8' }));
				};
				var canvas = doc.createElement('canvas'),
				    ctx = canvas.getContext('2d');
				getImg(this._iconEl.href, function (e) {
					var wh = e.target.naturalHeight;
					canvas.width = canvas.height = wh;
					ctx.drawImage(e.target, 0, 0, wh, wh);
					var url = svgToUrl(_this87._iconNew);
					getImg(url, function (e) {
						var original = ctx.getImageData(0, 0, wh, wh);
						ctx.drawImage(e.target, 0, 0, wh, wh);
						_this87._iconNew = canvas.toDataURL('image/png');
						ctx.putImageData(original, 0, 0);
						window.URL.revokeObjectURL(url);
						url = svgToUrl(_this87._iconYou);
						getImg(url, function (e) {
							ctx.drawImage(e.target, 0, 0, wh, wh);
							_this87._iconYou = canvas.toDataURL('image/png');
							ctx.putImageData(original, 0, 0);
							window.URL.revokeObjectURL(url);
							url = svgToUrl(_this87._iconError);
							getImg(url, function (e) {
								ctx.drawImage(e.target, 0, 0, wh, wh);
			...
			_iconError: '<path stroke="#780000" stroke-width="3" d="M15 15L7 7M7 15l8-8"/>' + '<path stroke="#FA2020" stroke-width="1.5" d="M14.5 14.5l-7-7M7.5 14.5l7-7"/>',

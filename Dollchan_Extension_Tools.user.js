...
function updateABUCap() {
	var cap = $id('captcha_div'), uwin = nav.Firefox ? unsafeWindow : window;
	if(!uwin.Recaptcha) {
...
function addTextButton(bbBrds, id) {
...
function appendPviewPanel(post, pView) {
	var cnt = post.count - (post.dcount || 0),
		...
		pText = (aib.getSage(post) ? '<span class="de-btn-sage" title="SAGE"></span>' : '') + (
			post.deleted ? '' :
				'<i style="vertical-align: 1px; color: #4f7942; font: bold 13px serif; cursor: ' +

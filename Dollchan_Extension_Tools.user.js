		...
		$New('div', [
			...
			$New('span', [
				...
				$if(!isMain, $new('span', {
					...
					'style': 'color:#fff; vertical-align:top; padding:0 3px; font-size:18px',
...
function tfBtn(id, title, wktag, bbtag, val, x) {
	...
	if(val != '&gt;') $event(btn, {'click': function(e) {
		...
		if(ch.hid && bbtag in {'code':0, 'u':0, 's':0, 'spoiler':0} || ch.nul || ch.so
			|| ch.krau || ch.sib || ch.zadr || (ch.fch && wktag == '%%')) {
			tag1 = '[' + bbtag + ']';
			tag2 = '[/' + bbtag + ']';
...
function funcPostPreview(post, parentId, msg) {
	...
	$Del('.//*[(starts-with(@id,"DESU_") or starts-with(@class,"DESU_")) and not(@class="DESU_postpanel")]', pView);
	eventRefLink(pView);
	addLinkTube(pView);

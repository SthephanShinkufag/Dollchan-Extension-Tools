...
(function deMainFuncInner(scriptStorage, FormData, scrollTo, localData) {
	...
	function showFavoritesWindow(body, favObj) {
		...
		btns.appendChild($btn(Lng.refresh[lang], Lng.infoCount[lang], _asyncToGenerator( regeneratorRuntime.mark(function _callee5() {
			var favObj, isUpdate, last404, myposts, els, i, len, el, host, _b, num, _f, _ref9, titleEl, youEl, countEl, iconEl, form, isArchived, _ref10, _ref11, bArch, posts, cnt, j, links, a, _len, tc;
				...
				if (id === 'common') {
					$after($q('input[info="userCSS"]').parentNode, getEditButton('css', function (fn) {
						return fn(Cfg.userCSSTxt, false, function () {
							saveCfg('userCSSTxt', _this15.value);
							updateCSS();
							toggleWindow('cfg', true);
						});
					}, 'de-cfg-button'));
			...
			if (!this.enabled) {
				...
				Promise.resolve(this.readKeys()).then(function (keys) {
					if (_this17.enabled) {
						...
						_this17.gKeys = _keys[2];
						_this17.ntKeys = _keys[3];
						_this17.tKeys = _keys[4];

						doc.addEventListener('keydown', _this17, true);
					}
				});
			}
		},
	...
	var ContentLoader = {
		...
		downloadThread: function downloadThread(imgOnly) {
			...
			this._thrPool = new TasksPool(4, function (num, data) {
				return _this18.loadImgData(data[0]).then(function (imgData) {
					...
					if (imgLink) {
						...
						if (imgData) {
							...
							if (imgOnly) {
								return _this18._getDataFromImg(el).then(function (data) {
									return tar.addFile(thumbName, data);
								}, emptyFn);
							}
						}
						return imgOnly ? null : _this18._getDataFromImg(el).then(function (data) {
							el.src = thumbName;
				...
				_this18._thrPool = tar = warnings = count = current = imgOnly = progress = counter = null;
			});
			els.forEach(function (el) {
				var imgLink = $parent(el, 'A');
				if (imgLink) {
					var _url = imgLink.href;
					_this18._thrPool.runTask([_url, imgLink.getAttribute('download') || _url.substring(_url.lastIndexOf('/') + 1), el, imgLink]);
				}
			});
			if (!imgOnly) {
				...
				$each($Q('link, *[src]', dc), function (el) {
					...
					files.push(fName);
					_this18._thrPool.runTask([url, fName, el, null]);
					count++;
				});
			}
		...
		preloadImages: function preloadImages(data) {
			...
			if (isPreImg || Cfg.preLoadImgs) {
				...
				preloadPool = new TasksPool(mReqs, function (num, data) {
					return _this19.loadImgData(data[0]).then(function (imageData) {
						...
						if (imageData) {
							...
							if (rarJpgFinder) {
								rarJpgFinder.runWorker(imageData.buffer, [imageData.buffer], function (info) {
									return _this19._addImgFileIcon(nameLink, fName, info);
								});
							}
						}
						if (_this19.popupId) {
							$popup(_this19.popupId, Lng.loadImage[lang] + ': ' + cImg + '/' + len, true);
						}
						cImg++;
					});
				}, function () {
					...
					if (_this19.afterFn) {
						_this19.afterFn();
						_this19.afterFn = _this19.popupId = null;
					}
					if (rarJpgFinder) {
...
true, true];
				...
				ajaxLoad(aib.getThrUrl(aib.b, this.tNum)).then(function (form) {
					...
					if (_this45.isOp) {
						...
						for (var i = 0, len = _els3.length; i < len; ++i) {
							if (_this45.num === aib.getPNum(_els3[i])) {
								_this45.updateMsg(aib.fixHTML(doc.adoptNode($q(aib.qPostMsg, _els3[i]))), maybeSpells.value);
								$del(el);
								break;
							}
						}
	...
	var Post = function (_AbstractPost) {
		...
		function Post(el, thr, num, count, isOp, prev) {
			...
			_this47.count = count;
			_this47.deleted = false;
			_this47.el = el;
			_this47.hidden = false;
			_this47.next = null;
			_this47.omitted = false;
			_this47.prev = prev;
			_this47.spellHidden = false;
			_this47.userToggled = false;
			_this47.viewed = false;
			...
			_this47.btns = $aEnd(_this47._pref = $q(aib.qPostRef, el), '<span class="de-post-btns' + (isOp ? '' : ' de-post-counter') + '">' + '<svg class="de-btn-hide"><use class="de-btn-hide-use" xlink:href="#de-symbol-post-hide"/>' + '<use class="de-btn-unhide-use" xlink:href="#de-symbol-post-unhide"/></svg>' + '<svg class="de-btn-rep"><use xlink:href="#de-symbol-post-rep"/></svg>' + (isOp ? (aib.t ? '' : '<svg class="de-btn-expthr"><use xlink:href="#de-symbol-post-expthr"/></svg>') + '<svg class="de-btn-fav"><use xlink:href="#de-symbol-post-fav"/></svg>' : '') + (_this47.sage ? '<svg class="de-btn-sage"><use xlink:href="#de-symbol-post-sage"/></svg>' : '') + '</span>');
			...
			value: function set(note) {
				...
				if (this.isHideThr) {
					...
					this._aEl.onclick = function (e) {
						$pd(e);
						_this50._post.setUserVisib(!_this50._post.hidden);
					};
						...
						while (1) {
							switch (_context17.prev = _context17.next) {
								case 0:
									...
									this.el.className = aib.cReply + ' de-pview' + (post.viewed ? ' de-viewed' : '') + (isMyPost ? ' de-mypost' : '');
									$show(this.el);
									$each($Q('.de-post-hiddencontent', this.el), function (el) {
										return el.classList.remove('de-post-hiddencontent');
									});
									if (Cfg.linksNavig) {
								...
								case 34:
									...
									_context17.t13 = post.deleted ? '' : '<span class="de-post-counter-pview">' + (this.isOp ? 'OP' : post.count + +!aib.JsonBuilder) + (isMyPost ? ' (You)' : '') + '</span>';
									...
									if (post instanceof CacheItem) {
										...
										this.btns.innerHTML = '<svg class="de-btn-' + (post.hidden ? 'unhide' : 'hide') + (post.userToggled ? '-user' : '') + ' de-btn-pview-hide" de-num="' + this.num + '"><!--\n\t\t\t\t--><use class="de-btn-hide-use" xlink:href="#de-symbol-post-hide"/><!--\n\t\t\t\t--><use class="de-btn-unhide-use" xlink:href="#de-symbol-post-unhide"/></svg>' + pText;
										$each($Q((!aib.t && post.isOp ? aib.qOmitted + ', ' : '') + '.de-fullimg-wrap, .de-fullimg-after', this.el), $del);
										$each($Q(aib.qPostImg, this.el), function (el) {
											return $show(el.parentNode);
										});
										link = $q('.de-link-parent', this.el);
										...
										if (Cfg.addYouTube && post.videos.hasLinks) {
											if (post.videos.playerInfo !== null) {
												Object.defineProperty(this, 'videos', { value: new Videos(this, $q('.de-video-obj', this.el), post.videos.playerInfo) });
											}
		...
		function Thread(el, num, prev, form) {
			...
			if (Cfg.hideReplies) {
				...
				repBtn.onclick = function (e) {
					$pd(e);
					var nextCoord = !_this62.next || _this62.last.omitted ? null : _this62.next.top;
			...
			value: function setFavorState(isEnable, preview) {
				...
				readFavorites().then(function (favObj) {
					if (isEnable) {
						...
						favObj[h][b][num] = { cnt: cnt, new: 0, you: 0, txt: txt, url: aib.getThrUrl(b, num), last: last };
					} else {
						removeFavEntry(favObj, h, b, num);
					}

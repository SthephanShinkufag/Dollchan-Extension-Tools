...
(function de_main_func_inner(scriptStorage, FormData) {
	...
	function pButton(id, href, hasHotkey) {
		return "<li><a id=\"de-panel-" + id + "\" class=\"de-abtn\" " + (hasHotkey ? "de-" : "") + "title=\"" + Lng.panelBtn[id][lang] + "\" href=\"" + href + "\"></a></li>";
	}
	...
	function addPanel(formEl) {
		var panel,
		    ...
		    isThr = aib.t;
		(pr && pr.pArea[0] || formEl).insertAdjacentHTML("beforebegin", "<div id=\"de-main\" lang=\"" + getThemeLang() + "\"><div id=\"de-panel\">" + "<span id=\"de-panel-logo\" title=\"" + Lng.panelBtn.attach[lang] + "\"></span>" + "<ul id=\"de-panel-btns\"" + (Cfg.expandPanel ? ">" : " style=\"display: none;\">") + (Cfg.disabled ? pButton("enable", "#", false) : pButton("cfg", "#", true) + pButton("hid", "#", true) + pButton("fav", "#", true) + (!Cfg.addYouTube ? "" : pButton("vid", "#", true)) + (localRun ? "" : pButton("refresh", "#", false) + (!isThr && aib.page === aib.firstPage ? "" : pButton("goback", aib.getPageUrl(aib.b, aib.page - 1), true)) + (isThr || aib.page === aib.lastPage ? "" : pButton("gonext", aib.getPageUrl(aib.b, aib.page + 1), true))) + pButton("goup", "#", false) + pButton("godown", "#", false) + (imgLen === 0 ? "" : pButton("expimg", "#", false) + pButton("maskimg", "#", true) + (nav.Presto || localRun ? "" : (Cfg.preLoadImgs ? "" : pButton("preimg", "#", false)) + (!isThr ? "" : pButton("savethr", "#", false)))) + (!isThr || localRun ? "" : pButton(Cfg.ajaxUpdThr ? "upd-on" : "upd-off", "#", false) + (nav.Safari ? "" : pButton("audio-off", "#", false))) + (!aib.mak && !aib.tiny && !aib.fch ? "" : pButton("catalog", aib.prot + "//" + aib.host + "/" + aib.b + "/catalog.html", false)) + pButton("enable", "#", false) + (!isThr ? "" : "<span id=\"de-panel-info\" title=\"" + Lng.panelBtn.counter[lang] + "\">" + dForm.firstThr.pcount + "/" + imgLen + "</span>")) + "</ul>" + "</div>" + (Cfg.disabled ? "" : "<div id=\"de-alert\"></div><hr style=\"clear: both;\">") + "</div>");
		panel = $id("de-panel");
		evtObject = {
			odelay: 0,
			panel: panel,
	...
	function toggleWindow(name, isUpd, data, isSync) {
		...
		if (!win) {
			main.insertAdjacentHTML("afterbegin", "<div id=\"de-win-" + name + "\" class=\"" + (Cfg[name + "WinDrag"] ? "de-win\" style=\"" + Cfg[name + "WinX"] + "; " + Cfg[name + "WinY"] : "de-win-fixed\" style=\"right: 0; bottom: 25px") + "; display: none;\">" + "<div class=\"de-win-head\"><span class=\"de-win-title\">" + (name === "cfg" ? "Dollchan Extension Tools" : Lng.panelBtn[name][lang]) + "</span>" + "<span class=\"de-win-buttons\">" + "<span class=\"de-btn-toggle\" title=\"" + Lng.toggleWindow[lang] + "\"></span>" + "<span class=\"de-btn-close\" title=\"" + Lng.closeWindow[lang] + "\"></span></span></div>" + "<div class=\"de-win-body" + (name === "cfg" ? " " + aib.cReply : "\" style=\"background-color: " + getComputedStyle(doc.body).getPropertyValue("background-color") + ";") + "\"></div></div>");
			win = main.firstChild;
	...
	function showWindow(win, name, isUpd, remove, data, isSync, isAnim) {
		...
		switch (name) {
			...
			case "hid":
				showHiddenTable(body);break;
			case "vid":
				showVideosTable(body);
		}
		if (isAnim) {
			win.classList.add("de-win-open");
		}
	...
	function showVideosTable(body) {
		...
		body.innerHTML = "<div de-disableautoplay class=\"de-video-obj\"></div>" + "<center>" + "<a class=\"de-abtn\" id=\"de-video-btn-prev\" href=\"#\" title=\"" + Lng.prevVideo[lang] + "\">" + "&#x25C0;</a> " + "<a class=\"de-abtn\" id=\"de-video-btn-hide\" href=\"#\" title=\"" + Lng.hideLnkList[lang] + "\">" + "&#x25B2;</a> " + "<a class=\"de-abtn\" id=\"de-video-btn-next\" href=\"#\" title=\"" + Lng.nextVideo[lang] + "\">" + "&#x25B6;</a></center>" + "<div id=\"de-video-list\" class=\"de-content-block\" style=\"max-width: " + (+Cfg.YTubeWidth + 40) + "px; max-height: " + (doc.documentElement.clientHeight - +Cfg.YTubeHeigh - 110) + "px;\"></div>";
	...
	function showFavoriteTable(body, data) {
		for (var h in data) {
			for (var b in data[h]) {
				...
				for (var tNum in d) {
					...
					block.insertAdjacentHTML("beforeend", "<div class=\"de-entry " + aib.cReply + "\" de-host=\"" + h + "\" de-board=\"" + b + "\" de-num=\"" + tNum + "\" de-url=\"" + t.url + "\">" + "<div>" + (t.type !== "user" ? "" : "<span class=\"de-fav-user\" title=\"" + Lng.setByUser[lang] + "\"></span>") + "<input type=\"checkbox\">" + "<a href=\"" + t.url + (t.last ? aib.anchor + t.last : "") + "\">" + tNum + "</a></div>" + "<div class=\"de-fav-title\">&nbsp;- " + t.txt + "</div>" + "<div class=\"de-fav-inf\">" + "<span class=\"de-fav-inf-err\">" + (t.err || "") + "</span> " + "<span class=\"de-fav-inf-new\" title=\"" + Lng.newPosts[lang] + "\"" + (t["new"] ? ">" : " style=\"display: none;\">") + (t["new"] || 0) + "</span> " + "[<span class=\"de-fav-inf-old\" title=\"" + Lng.oldPosts[lang] + "\">" + t.cnt + "</span>] " + "<span class=\"de-fav-inf-page\" title=\"" + Lng.thrPage[lang] + "\"></span>" + "</span></div>");
				}
			}
		}
		...
		body.appendChild($btn(Lng.page[lang], Lng.infoPage[lang], async(regeneratorRuntime.mark(function callee$2$1() {
		...
		body.appendChild($btn(Lng.clear[lang], Lng.clrDeleted[lang], async(regeneratorRuntime.mark(function callee$2$2() {
	...
	function PostForm(form, ignoreForm, dc) {
		...
		el = this.qArea;
		el.insertAdjacentHTML("beforeend", "<div class=\"de-resizer-top\"></div>" + "<div class=\"de-win-head\">" + "<span class=\"de-win-title\"></span>" + "<span class=\"de-win-buttons\">" + "<span class=\"de-btn-toggle\" title=\"" + Lng.toggleReply[lang] + "\"></span>" + "<span class=\"de-btn-close\" title=\"" + Lng.closeReply[lang] + "\"></span></span></div>" + "<div class=\"de-resizer-left\"></div>" + "<div class=\"de-resizer-right\"></div>" + "<div class=\"de-resizer-bottom\"></div>");
		...
		el.firstChild.onclick = function () {
			...
			if (Cfg.replyWinDrag) {
				_this.qArea.className = aib.cReply + " de-win";
				updateWinZ(_this.qArea.style);
			} else {
				_this.qArea.className = aib.cReply + " de-win-inpost";
				_this.txta.focus();
			}
		};
		...
		handleEvent: function handleEvent(e) {
			...
			switch (e.type) {
				case "mousedown":
					switch (this.dir) {
						case "top":
							val = Cfg.replyWinX + "; bottom: " + (maxY - cr.bottom) + "px";break;
						case "bottom":
							val = Cfg.replyWinX + "; top: " + cr.top + "px";break;
						case "left":
							val = "right: " + (maxX - cr.right) + "px; " + Cfg.replyWinY;break;
						case "right":
							val = "left: " + cr.left + "px; " + Cfg.replyWinY;
					}
				...
				default:
					...
					if (this.vertical) {
						...
						saveCfg("replyWinY", cr.top < 1 ? "top: 0" : cr.bottom > maxY - 26 ? "bottom: 25px" : "top: " + cr.top + "px");
					} else {
						saveCfg("textaWidth", parseInt(this.txStyle.width, 10));
						saveCfg("replyWinX", cr.left < 1 ? "left: 0" : cr.right > maxX - 1 ? "right: 0" : "left: " + cr.left + "px");
					}
	...
	function getImageBoard(checkDomains, checkEngines) {
		...
		var ibDomains = Object.defineProperties({
			...
			"dobrochan.com": [{
				...
				css: { value: ".delete > img, .popup, .reply_, .search_google, .search_iqdb { display: none; }\t\t\t\t.delete { background: none; }\t\t\t\t.delete_checkbox { position: static !important; }\t\t\t\t.file + .de-video-obj { float: left; margin: 5px 20px 5px 5px; }\t\t\t\t.de-video-obj + div { clear: left; }" },
			...
			"krautchan.net": [{
				...
				css: { value: "img[src$=\"button-expand.gif\"], img[src$=\"button-close.gif\"], body > center > hr, form > div:first-of-type > hr, h2, .sage { display: none; }\t\t\t\tdiv[id^=\"Wz\"] { z-index: 10000 !important; }\t\t\t\t.de-thr-hid { float: none; }\t\t\t\t.file_reply + .de-video-obj, .file_thread + .de-video-obj { margin: 5px 20px 5px 5px; float: left; }\t\t\t\t.de-video-obj + div { clear: left; }\t\t\t\tform[action=\"/paint\"] > select { width: 105px; }\t\t\t\tform[action=\"/paint\"] > input[type=\"text\"] { width: 24px !important; }" },
				fixFileInputs: { value: function value(el) {
						var str = "";
		...
		var ibEngines = Object.defineProperties({
			...
			"form[action*=\"futaba.php\"]": {
				...
				cssEn: { value: ".ftbl { width: auto; margin: 0; }\t\t\t\t.reply { background: #f0e0d6; }\t\t\t\tspan { font-size: inherit; }" },
				docExt: { value: ".htm" },
				thrid: { value: "resto" }
			},
			"form[action*=\"imgboard.php?delete\"]": {
	...
	function updateCSS() {
		...
		x += ".postarea, .recaptcha_image_cell + td, .recaptcha_image_cell + td + td, small[id^=\"rfmap\"], .theader, .thumbnailmsg, " + (Cfg.panelCounter ? "" : "#de-panel-info, ") + (Cfg.imgNavBtns ? "" : "#de-img-btn-next, #de-img-btn-prev, ") + (Cfg.showHideBtn ? "" : ".de-btn-hide, ") + (Cfg.showRepBtn ? "" : ".de-btn-rep, ") + (Cfg.updThrBtns || aib.t ? "" : ".de-thread-updater, ") + (Cfg.removeHidd ? ".de-link-ref.de-link-hid, .de-link-ref.de-link-hid + .de-refcomma, " : "") + (Cfg.delHiddPost ? ".de-thr-hid, .de-thr-hid + div + hr, .de-thr-hid + div + br, .de-thr-hid + div + br + hr, .de-thr-hid + div + div + hr, " : "") + (Cfg.noPostNames ? aib.qName + ", ." + aib.cTrip + ", " : "") + (Cfg.noBoardRule ? (aib.mak ? ".rules-area" : aib.krau ? "#rules_row" : aib.futa ? ".chui" : ".rules, #rules") + ", " : "") + (!aib.kus && (aib.multiFile || !Cfg.fileThumb) ? "#de-pform form > table > tbody > tr > td:not([colspan]):first-child, #de-pform form > table > tbody > tr > th:first-child, " : "") + "body > hr { display: none !important; }";

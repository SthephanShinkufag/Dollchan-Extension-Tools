...
function imgDesaturate(data, w, h) {
	var i, pix,
		len = w * h,
		rv = new Uint8Array(len);
	for(i = 0, pix = 0; i < len; i++, pix += 4) {
		rv[i] =
		// Average
		//	(data[pix] + data[pix1] + data[pix2]) / 3;
		// Scaled
			(data[pix] * 0.3 + data[pix + 1] * 0.59 + data[pix + 2] * 0.11);
	}
	return rv;
}
...
function imgPosterize(data, w, h) {
	...
	var levels = 2,
		...
		values = 256 / (levels - 1),
		i, len, a;
	for(i = 0, len = w * h; i < len; i++) {
		a = values * ((data[i] / areas) >> 0);
		if(a > 255) a = 255;
		data[i] = a;
	}
}
...
function getCanvasByData(data, w, h) {
	...
	while(i--) {
		cti.data[i * 4] = data[i];
		cti.data[i * 4 + 1] = data[i];
		cti.data[i * 4 + 2] = data[i];
		cti.data[i * 4 + 3] = 255;
	}

...
function addPanel() {
	...
	(pr && pr.pArea[0] || dForm).insertAdjacentHTML('beforebegin',
		'<div id="MAKAKA" lang="' + getThemeLang() + '">' +
			'<div id="de-panel">' +
...
function loadDocFiles(imgOnly) {
	...
	if (!imgOnly) {
		files = [];
		$each($Q('#MAKAKA, .de-parea, .de-post-btns, #de-qarea, .de-refmap, #de-updater-div, .de-video-obj,' +
...
function scriptCSS() {
	...
	// Settings window
	x += '#MAKAKA { -moz-box-sizing: content-box; box-sizing: content-box; }\

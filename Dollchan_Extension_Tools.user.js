...
(function de_main_func_inner(scriptStorage, FormData) {
		...
		LoggerSingleton.prototype = {
			...
			getTable: function getTable() {
				...
				for (var i = 0, len = data.length; i < len; ++i) {
					html += "<div class=\"de-info-row\"><span class=\"de-info-name\">" + data[i][0] + "</span><span>" + data[i][1] + "ms</span></div>";
				}
				return html;
			},
			init: function init() {
		...
		for (var h in data) {
			for (var b in data[h]) {
				...
				for (var tNum in d) {
					...
					block.insertAdjacentHTML("beforeend", "<div class=\"de-entry " + aib.cReply + "\" de-host=\"" + h + "\" de-board=\"" + b + "\" de-num=\"" + tNum + "\" de-url=\"" + t.url + "\">" + (t.type !== "user" ? "" : "<span class=\"de-fav-user\" title=\"" + Lng.setByUser[lang] + "\"></span>") + "<input type=\"checkbox\">" + "<a href=\"" + t.url + (!t.last ? "" : t.last.startsWith("#") ? t.last : h === aib.host ? aib.anchor + t.last : "") + "\">" + tNum + "</a>" + "<div class=\"de-entry-title\">- " + t.txt + "</div>" + "<div class=\"de-fav-inf\">" + "<span class=\"de-fav-inf-err\">" + (t.err || "") + "</span> " + "<span class=\"de-fav-inf-new\" title=\"" + Lng.newPosts[lang] + "\"" + (t["new"] ? ">" : " style=\"display: none;\">") + (t["new"] || 0) + "</span> " + "[<span class=\"de-fav-inf-old\" title=\"" + Lng.oldPosts[lang] + "\">" + t.cnt + "</span>] " + "<span class=\"de-fav-inf-page\" title=\"" + Lng.thrPage[lang] + "\"></span>" + "</span></div>");
	...
	function getCfgInfo() {
		...
		return $New("div", { "class": "de-cfg-unvis", id: "de-cfg-info" }, [$add("<div style=\"padding-bottom: 10px;\">" + "<a href=\"https://github.com/SthephanShinkufag/Dollchan-Extension-Tools/wiki/versions\" " + "target=\"_blank\">v" + version + "." + commit + "</a>&nbsp;|&nbsp;" + "<a href=\"http://www.freedollchan.org/scripts/\" target=\"_blank\">Freedollchan</a>&nbsp;|&nbsp;" + "<a href=\"https://github.com/SthephanShinkufag/Dollchan-Extension-Tools/wiki/" + (lang ? "home-en/" : "") + "\" target=\"_blank\">Github</a></div>"), $add("<div id=\"de-info-table\"><div class=\"de-info-column\">" + "<div class=\"de-info-row\"><span class=\"de-info-name\">" + Lng.thrViewed[lang] + "</span><span>" + Cfg.stats.view + "</span></div>" + "<div class=\"de-info-row\"><span class=\"de-info-name\">" + Lng.thrCreated[lang] + "</span><span>" + Cfg.stats.op + "</span></div>" + "<div class=\"de-info-row\"><span class=\"de-info-name\">" + Lng.thrHidden[lang] + "</span><span>" + getHiddenThrCount() + "</span></div>" + "<div class=\"de-info-row\"><span class=\"de-info-name\">" + Lng.postsSent[lang] + "</span><span>" + Cfg.stats.reply + "</span></div></div>" + "<div class=\"de-info-column\" style=\"overflow-y: auto; border-left: 1px solid grey;\">" + new Logger().getTable() + "</div></div>"), $btn(Lng.debug[lang], Lng.infoDebug[lang], function () {
	...
	Videos.addPlayer = function (el, m, isYtube) {
		...
		el.lastChild.onclick = function () {
			...
			if (node.parentNode.parentNode.id === "de-win-vid") {
				...
				node.style.maxHeight = doc.documentElement.clientHeight - (exp ? 590 : +Cfg.YTubeHeigh + 155) + "px";
			}
		};
	};
